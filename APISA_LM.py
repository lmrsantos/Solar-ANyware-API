...
def createxml(xmlfile, sitename, latitude, longitude):
    ...
    userName = ##[add user name to login Solar Anyware]
    password = ##[add password to access Solar Anyware account]
...
def convertxmlcsv(filedir,sitename,latitude,longitude):
    # xml_dir = filedir+sitename+" ("+latitude+","+longitude+").xml"
    # csv_dir = filedir+sitename+" ("+latitude+","+longitude+").csv"
    #    xml_file = xml_dir
    

    xml_dir = filedir+sitename+"("+latitude+","+longitude+") SolarAnywhere Typical GHI Year SA format-RPA.xml"
    csv_dir = filedir+sitename+"("+latitude+","+longitude+") SolarAnywhere Typical GHI Year SA format-RPA.csv"
    ...
    with open(csv_dir, 'w', newline='') as csvfile:
        ...
        first_line="0,("+latitude+" "+longitude+"),"+sitename+","+tz_utc+","+latitude+","+longitude+","+elevation+",Data Version: "+Data_Ver+" / Type: Typical Year / LatLon Resolution: 0.100 / Time Resolution: "+timeresolution+" minutes / Averaging Method: End of Period / Copyright 2010-2020 Clean Power ResearchÂ®,L.L.C. DownloadID="+weatherrequestid+"\n"
        ...
        for wdp in root[0][0][1]:
            ...
            ObsObjDate = datetime.strptime(ObsStrDate,'%Y-%m-%dT%H:%M:%S-08:00') + timedelta(hours=0, minutes=30)
            ...
            # ObservationTime_GMT = datetime.strftime(datetime.fromisoformat(wdp.attrib.get('ObservationTime'))+timedelta(hours=0, minutes=30),'%m/%d/%Y %H:%M')
...
if __name__ == "__main__":
    sitename="Reno"
    latitude="39.6017736" 
    longitude="-119.8033576"
    filedir = # [add directory name where the files must read/write]
    xml = filedir+sitename+" ("+latitude+","+longitude+").xml"
    csv = filedir+sitename+" ("+latitude+","+longitude+").csv"
    print(sitename,"\n",latitude,"\n",longitude)
